...
async def test_snowflake_id_two_generators_32bit():
    ...
    assert id1 != id2, (
        "IDs from different generator configurations should be different."
    )
...
async def test_async_snowflake_sequence_reset_32bit():
    ...
    assert info1.timestamp_ms < info2.timestamp_ms, (
        "Timestamp of id2 should be greater than id1."
    )  # Attribute access
    if info1.timestamp_ms < info2.timestamp_ms:
        assert info2.sequence == 0, (
            "Sequence should reset for a new millisecond."
        )  # Attribute access
...
async def test_async_extract_snowflake_info_32bit():
    ...
    snowflake_id = await generator.generate()
    info: SnowflakeInfo = generator.extract_snowflake_info(
        snowflake_id
    )  # Added type hint for info

    assert info.timestamp_ms is not None, (
        "Timestamp (ms) should be extracted."
    )  # Attribute access
    assert info.timestamp_ms >= current_time_ms - 50, (
        "Extracted timestamp_ms should be around current time at start of test."
    )  # Attribute access
    assert info.timestamp_ms < current_time_ms + 500, (
        "Extracted timestamp_ms is too far in the future."
    )  # Attribute access
    assert info.worker_id == TEST_CONFIG_32BIT.worker_id, (
        "Incorrect worker ID extracted."
    )  # Attribute access
    assert info.node_id == TEST_CONFIG_32BIT.node_id, (
        "Incorrect node ID extracted."
    )  # Attribute access
...
def test_sync_extract_snowflake_info_32bit():
    ...
    assert info.timestamp_ms >= current_time_ms, (
        "Extracted timestamp_ms should be >= current time at start of test."
    )  # Attribute access
    assert info.timestamp_ms < current_time_ms + 500, (
        "Extracted timestamp_ms is too far in the future."
    )  # Attribute access
